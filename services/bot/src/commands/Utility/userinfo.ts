module.exports = {
  name:"userinfo",aliases:["ui","whois"],category:"Utility",description:"Info detail user Discord",usage:"userinfo [@user]",
  execute:async(message:any,_:any,client:any)=>{
    const t=message.mentions.members?.first()??message.member
    const roles=t.roles.cache.filter((r:any)=>r.id!==message.guild.id).sort((a:any,b:any)=>b.position-a.position).map((r:any)=>r.toString()).slice(0,5)
    client.embed().setAuthor({name:t.user.tag,iconURL:t.user.displayAvatarURL()}).setThumbnail(t.user.displayAvatarURL({size:256}))
    message.reply({embeds:[client.embed()
      ...
      .addFields(
        {name:"🆔 ID",value:"`"+t.user.id+"`",inline:true},{name:"🤖 Bot",value:t.user.bot?"Ya":"Tidak",inline:true},
