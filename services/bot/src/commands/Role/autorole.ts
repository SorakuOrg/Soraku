...
import { Autorole } from "../../schema/db"
module.exports = {
  ...
  execute:async(message:any,args:string[],client:any)=>{
    if(!message.member.permissions.has(PermissionFlagsBits.Administrator))
      return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Butuh izin **Administrator**.").setFooter({text:"Soraku Community"})]})
    const type=args[0]?.toLowerCase()
    const opt=args[1]?.toLowerCase()
    const role=message.mentions.roles.first()
    if(!type||!opt){
      const cfg=await Autorole.get(message.guild.id) as any
      return message.reply({embeds:[client.embed().setTitle("🎭 Autorole Config")
        .addFields({name:"👤 Human Roles",value:(cfg?.human_roles||[]).map((id:string)=>"<@&"+id+">").join(" ")||"Tidak ada",inline:false},{name:"🤖 Bot Roles",value:(cfg?.bot_roles||[]).map((id:string)=>"<@&"+id+">").join(" ")||"Tidak ada",inline:false})
    ...
    const cfg=await Autorole.get(message.guild.id) as any||{human_roles:[],bot_roles:[]}
    const key=type==="human"?"human_roles":"bot_roles"
    const list:string[]=cfg[key]||[]
