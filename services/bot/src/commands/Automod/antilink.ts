...
module.exports = {
  name: "antilink", aliases: ["al"], category: "Automod",
  description: "Konfigurasi antilink — hapus otomatis link Discord", usage: "antilink <enable|disable>",
  execute: async (message: any, args: string[], client: any) => {
    ...
    const cfg = await Antilink.get(message.guild.id) as any
    if (!opt) {
      return message.reply({ embeds: [
        client.embed().setTitle("🤖 Antilink").addFields({ name: "Status", value: cfg?.is_enabled ? "✅ Aktif" : "❌ Nonaktif", inline: true })
