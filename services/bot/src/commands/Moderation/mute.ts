...
module.exports = {
  ...
  execute:async(message:any,args:string[],client:any)=>{
    if(!message.member.permissions.has(PermissionFlagsBits.ModerateMembers))
      return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Butuh izin **Moderate Members**.").setFooter({text:"Soraku Community"})]})
    ...
    message.reply({embeds:[client.embed().setTitle("🔇 Member Dimute").addFields({name:"👤 User",value:t.user.tag,inline:true},{name:"⏱️ Durasi",value:mins+" menit",inline:true},{name:"📋 Alasan",value:reason}).setFooter({text:"Soraku Community"}).setTimestamp()]})
  }
}