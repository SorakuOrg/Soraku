...
module.exports = {
  name:"vcmuteall",category:"Voice",description:"Server mute semua user di VC bot",usage:"vcmuteall",
  execute:async(message:any,_:any,client:any)=>{
    ...
    let count=0
    for(const [,m] of vc.members){if(!m.user.bot){await m.voice.setMute(true).catch(()=>{});count++}}
