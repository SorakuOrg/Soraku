module.exports = {
  ...
  execute: async (interaction: any, client: any) => {
    ...
    try {
      ...
      events = ((await r.json()) as any).data ?? []
    } catch {}
    const embed = client.embed()
      .setTitle("🎌 Event Soraku yang Akan Datang")
    ...
    if (!events.length) { embed.setDescription("Belum ada event mendatang. Pantau terus ya~") }
      ...
      embed.addFields({ name: ev.title, value: `📅 <t:${Math.floor(new Date(ev.startdate).getTime()/1000)}:F>\n🔗 [Detail](${client.webUrl}/events/${ev.slug})`, inline: false })
    }
