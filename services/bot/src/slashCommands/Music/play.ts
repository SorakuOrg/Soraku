module.exports = {
  ...
  execute:async(interaction:any,client:any)=>{
    if(!client.manager)return interaction.reply({content:"❌ Musik tidak aktif di server ini.",ephemeral:true})
    if(!interaction.member.voice.channel)return interaction.reply({content:"❌ Kamu harus di voice channel.",ephemeral:true})
