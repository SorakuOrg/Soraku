...
function makeRow(paused: boolean) {
...
module.exports = {
  name: "playerStart",
  run: async (client: SorakuClient, player: any, track: any) => {
    ...
    const msg = await (channel as any).send({ embeds: [embed], components: [makeRow(false)] }).catch(() => null)
    if (!msg) return
