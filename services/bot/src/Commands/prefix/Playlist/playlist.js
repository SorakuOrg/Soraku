...
module.exports = {
  ...
  execute:async(message,args,client)=>{
    const opt=args[0]?.toLowerCase();const name=args.slice(1).join(" ")
    if(!opt||opt==="list"){
      const lists=await Playlist.getAll(message.author.id)
      if(!lists.length)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Buat playlist dulu: `!playlist create <nama>`").setFooter({text:"Soraku Community"})]})
      return message.reply({embeds:[client.embed().setTitle(client.emoji.queue+" Playlist Kamu").setDescription(lists.map((p,i)=>"`"+(i+1)+"`. **"+p.name+"** ("+(p.tracks?.length||0)+" lagu)").join("\n")).setFooter({text:"Soraku Community"})]})
    }
    if(opt==="create"){
      ...
      return message.reply({embeds:[client.embed().setDescription(client.emoji.tick+" Playlist **"+name+"** dibuat.").setFooter({text:"Soraku Community"})]})
    }
    if(opt==="delete"){
      if(!name)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Sebutkan nama playlist.").setFooter({text:"Soraku Community"})]})
    ...
    if(opt==="save"){
      ...
      if(!player?.queue.current)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Tidak ada lagu.").setFooter({text:"Soraku Community"})]})
      if(!name)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Sebutkan nama playlist.").setFooter({text:"Soraku Community"})]})
      const pl=await Playlist.get(message.author.id,name)
      ...
      const tracks=[player.queue.current,...player.queue].map(t=>({title:t.title,uri:t.uri,length:t.length}))
    ...
    if(opt==="load"){
      if(!client.manager)return message.reply({embeds:[client.embed().setDescription("❌ Musik tidak aktif.").setFooter({text:"Soraku Community"})]})
      if(!message.member.voice.channel)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Masuk VC dulu.").setFooter({text:"Soraku Community"})]})
      if(!name)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Sebutkan nama playlist.").setFooter({text:"Soraku Community"})]})
      const pl=await Playlist.get(message.author.id,name)
      if(!pl?.tracks?.length)return message.reply({embeds:[client.embed().setDescription(client.emoji.cross+" Playlist kosong atau tidak ditemukan.").setFooter({text:"Soraku Community"})]})
      ...
      for(const t of pl.tracks){const r=await player.search(t.uri,{requester:message.author}).catch(()=>null);if(r?.tracks[0])player.queue.add(r.tracks[0])}
