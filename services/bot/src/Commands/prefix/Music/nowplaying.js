...
module.exports = {
  ...
  execute:async(message,_,client)=>{
    ...
    if(!song)return message.reply({embeds:[client.embed().setDescription("❌ Tidak ada lagu.").setFooter({text:"Soraku Community"})]})
    const bar=progressbar(player.position,song.length)
    message.reply({embeds:[client.embed().setTitle(client.emoji.music+" Now Playing")
