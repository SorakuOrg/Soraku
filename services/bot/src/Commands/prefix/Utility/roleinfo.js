module.exports = {
  name: "roleinfo", aliases: ["ri"], category: "Utility",
  description: "Info detail sebuah role", usage: "roleinfo @role",
  execute: async (message, _args, client) => {
    ...
    await message.reply({ embeds: [
      client.embed().setTitle(`🎭 ${role.name}`).setColor(role.color || 0x7c3aed)
        .addFields(
          { name: "🆔 ID",          value: `\`${role.id}\``,   inline: true },
          { name: "🎨 Warna",       value: `\`${role.hexColor}\``, inline: true },
          { name: "👥 Member",      value: String(role.members.size), inline: true },
          { name: "📌 Posisi",      value: String(role.position), inline: true },
