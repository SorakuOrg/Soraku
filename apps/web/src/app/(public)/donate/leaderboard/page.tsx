  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          {PODIUM.map((idx, i) => {
            ...
            return (
              <div key={d.id} className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs font-bold text-center max-w-[80px] truncate">{d.displayname}</p>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground/60 mb-5">Semua Donatur</h2>
        {sorted.map((d, i) => {
          const style = TIER_STYLES[d.tier ?? "DONATUR"];
          return (
            <div key={d.id} className={`glass-card flex items-center gap-4 p-4 ${style.glow}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground/50 mt-0.5">{formatDate(d.createdat)}</p>
              </div>
      ...
      <div className="mt-12 glass-card px-8 py-8 text-center">
        ...
        <div className="flex justify-center gap-3 flex-wrap">
          <Link href="/donate" className="rounded-xl bg-rose-600 px-6 py-3 text-sm font-bold text-white hover:bg-rose-700 transition-colors">
            Donasi via Trakteer
          </Link>
          <Link href="/premium" className="rounded-xl border border-border px-6 py-3 text-sm font-medium text-muted-foreground hover:border-primary/40 hover:text-foreground transition-colors">
            Premium Membership
          </Link>
        </div>
      </div>
