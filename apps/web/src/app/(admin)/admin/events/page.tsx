...
interface Event {
  ...
  starts_at: string; event_type: string;
}
          ...
          {events.map(event => {
            ...
            return (
              <div key={event.id} className="glass-card p-5 flex items-start gap-4">
                <div className={`mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl ${isUpcoming ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                  <TypeIcon className="h-4 w-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-sm font-medium line-clamp-1">{event.title}</h3>
                    <span className={`flex-shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${isUpcoming ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                      {isUpcoming ? "Upcoming" : "Selesai"}
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">{fmt(event.starts_at)}</p>
                </div>
